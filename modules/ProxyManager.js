...
module.exports = class ProxyManager {
  ...
  static async sendRequest(
    ...
    const madeRequest = async (opts) => {
      ...
      if (
        !url.includes("api") && !url.includes(".soundcloud.com/") &&
        !url.includes("tracks")
      ) {
        obj.success = true;
        return resp;
    ...
    const proxyPromises = proxiesFiltered.map((proxy) =>
      madeRequest({ ...init, agent: agentFor(proxy) })
    );
    proxyPromises.push(madeRequest({ ...init }));

    const abortAll = () =>
      controllers.forEach((c) => {
        if (!c.success) c.abort.abort();
      });
    ...
    try {
      ...
      if (aggregate && aggregate.errors) {
        let err = aggregate.errors.reverse().find((e) =>
  ...
  static async Init(nmanager) {
    ...
    if (proxyList.length === 0) {
      const notify = new Notify(
        "SoundCloud",
        translations.proxy_available_not_found,
        10,
        __dirname + "/../icons/appLogo.png",
      );
    ...
    for (let i = 0; i < proxyList.length; i++) {
      ...
      if (typeof proxyConfig == "string") {
        ...
        proxy = ParseProxy(
          proxyConfig.url,
          proxyConfig.bestBypass === true,
          proxyConfig.dontCheck === true,
        );
    ...
    electron.app.on(
      "login",
      async (ev, webContents, req, authInfo, callback) => {
        if (!authInfo.isProxy) {
          return;
        }

        const proxy = allProxies.find((x) =>
          x.host === authInfo.host + ":" + authInfo.port && x.auth
        );
        if (!proxy) {
          return;
        }
    ...
    setInterval(async () => {
      ...
      if (
        ...
        (work.length === 0 || this.previosCount === 0)
      ) {
        if (work.length === 0) {
          const notify = new Notify(
            "SoundCloud",
            translations.proxy_work_not_found,
            10,
          );
          nmanager.show(notify);
        } else {
          const notify = new Notify(
            ...
            "",
            10,
            __dirname + "/../icons/data-server.png",
          );
          notify.body = translations.proxy_connected.replaceAll(
            "{name}",
            "<br>" + work.map((x) => x.name).join(";<br>") + ";",
          );
          nmanager.show(notify);
        }
    ...
    if (workProxies.length === 0) {
      ...
      notify.body = translations.proxy_connected.replaceAll(
        "{name}",
        "<br>" + workProxies.map((x) => x.name).join(";<br>") + ";",
      );
      nmanager.show(notify);
    }
  ...
  return new Promise(async (resolve) => {
    ...
    nfetch("https://soundcloud.com", proxyJson)
      .then(async (res) => {
        ...
        if (
          content.includes("The Squid Software") ||
          !content.includes('meta content="SoundCloud"')
        ) {
          resolve(false);
          _sended = true;
          return;
