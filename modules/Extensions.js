...
const rulang = _localize.includes("ru") || _localize.includes("kk") ||
  _localize.includes("ky") || _localize.includes("be");
...
module.exports = class Extensions {
  ...
  static translationsProxy() {
    const translated = rulang
      ? config.translations.ru
      : (config.translations[_localize] ?? config.translations.en);
    const _default = {
    ...
    if (translated) {
      if (typeof (translated.proxy_available_not_found) == "string") {
        _default.proxy_available_not_found =
          translated.proxy_available_not_found;
      }
