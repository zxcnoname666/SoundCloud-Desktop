...
# Протокол приложения
> Вы можете использовать протокол `sc://` для перемещения по url-адресам внутри приложения

> Для этого надо заменить `https://soundcloud.com/...` на `sc://...`. По принципу: `https://soundcloud.com/discover` => `sc://discover`

> В приложении также доступно перемещение по вводу ссылки (как в браузере)
...
`proxy` - это массив строк ip:port доступных прокси, а также работающих через http (например `1.1.1.1:80` или `user:password@ip:port`)
