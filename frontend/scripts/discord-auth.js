    ...
    try {
        if (typeof ipcRenderer !== 'undefined') {
            ...
            if (t && t.discord) {
                ...
                throw new Error('No discord translations');
            }
        }
    } catch (error) {
        discordTranslations = { // maybe need to change this ai generated guide idk
...
function updateDiscordModalUI() {
    ...
    if (!statusIndicator) {
        throw new Error('no statusIndicator');
    }
    if (!discordTranslations) {
        throw new Error('no discordTranslations');
    }
    ...
    `;
        ...
        <div class="discord-user-details">
          <span class="discord-username">${discordUser.username}</span>
        </div>
      `;
