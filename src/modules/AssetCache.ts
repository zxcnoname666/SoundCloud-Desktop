...
export class AssetCache {
   ...
   */
  async get(url: string): Promise<NodeFetchResponse | null> {
    ...
    try {
      ...
      // Создаем mock объект Response для node-fetch
      const response = {
        url: cached.url,
   ...
   */
  async set(url: string, response: NodeFetchResponse): Promise<void> {
    if (!this.enabled || !this.isStaticAsset(url)) {
      return;
    }
    ...
    try {
      ...
      if (!this.isCacheableResponse(headersObj)) {
      ...
      console.log(`💾 Cache SET: ${url} (${Math.round(bodyBuffer.length / 1024)}kb)`);
    } catch (error) {
